...
const  {kaf_channel} = require('../configs/local')
const producer = require("../middleware/producer");
const{moveConsentToDeleteConsent,
    ...
    try {
        ...
        if (rs.length > 0) {
            ...
            for (let a = 0; a < arrayMessageValue.length; a++) {
                ...
                const readableString = util.inspect(arrayMessageValue, { depth: 4, colors: 0 }); 
                ...
                return await producer(readableString[a].tran_id, messageSendProducer);
            }
        }
        pool.end();
        return 1; // Success
